...
class TestActivitydatesAPIFilter(APITestCase, ExtractPaginationMixin):
    ...
    def test_filter_after_date(self):
        ...
        selected_activities = ActivityModel.objects.filter(place__group__members=self.member).filter(
            date__startswith__gte=query_date
        )
    ...
    def test_filter_before_date(self):
        ...
        selected_activities = ActivityModel.objects.filter(place__group__members=self.member).filter(
            date__startswith__lte=query_date
        )
        self.assertEqual(len(response.data), selected_activities.count())
...
class TestCustomActivityFilters(APITestCase, ExtractPaginationMixin):
    def setUp(self):
        ...
        self.activity_feedback_dismissed.activityparticipant_set.filter(user=self.member).update(
    ...
    def test_filter_joined(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(
            {activity["id"] for activity in response.data},
            {
                self.activity_feedback_possible.id,
                self.activity_upcoming.id,
                self.activity_too_long_ago.id,
                self.activity_feedback_already_given.id,
