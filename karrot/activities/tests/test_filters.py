...
class TestActivityFilters(TestCase):
    ...
    def test_with_no_parameters(self):
        ...
        self.assertEqual(list(f.qs), [activity])
    ...
    def test_with_date_min_during_activity(self):
        activity = Activity.objects.create(place=self.place)
        self.expect_results(
            date_min=halfway_datetime(activity.date),
            results=[activity],
        )
    ...
    def test_with_date_max_during_activity(self):
        activity = Activity.objects.create(place=self.place)
        self.expect_results(
            date_max=halfway_datetime(activity.date),
            results=[activity],
        )
    ...
    def test_with_date_max_after_activity(self):
        activity = Activity.objects.create(place=self.place)
        self.expect_results(
            date_max=activity.date.end + timedelta(hours=2),
            results=[activity],
        )
    ...
    def test_no_results_with_date_max_before_activity(self):
        activity = Activity.objects.create(place=self.place)
        self.expect_results(
            date_max=activity.date.start - timedelta(hours=2),
            results=[],
        )
    ...
    def test_with_date_max_and_min_during_activity(self):
        activity = Activity.objects.create(place=self.place)
        self.expect_results(
            date_min=activity.date.start + timedelta(seconds=5),
            date_max=activity.date.end - timedelta(seconds=5),
            results=[activity],
        )
    ...
    def test_with_date_max_and_min_overlapping_activity_start(self):
        ...
        self.expect_results(
            date_min=activity.date.start - timedelta(seconds=5),
            date_max=activity.date.start + timedelta(seconds=5),
            results=[activity],
        )
    ...
    def test_with_date_max_and_min_overlapping_activity_end(self):
        activity = Activity.objects.create(place=self.place)
        self.expect_results(
            date_min=activity.date.end - timedelta(seconds=5),
            date_max=activity.date.end + timedelta(seconds=5),
            results=[activity],
        )
