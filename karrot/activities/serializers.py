...
class FeedbackSerializer(serializers.ModelSerializer):
    ...
    def validate_about(self, about):
        ...
        if not group.is_member(user):
            raise ValidationError("You are not member of the place's group.")
        if about.is_upcoming():
            raise ValidationError("The activity is not done yet")
        ...
        if no_shows:
            ...
            for no_show in no_shows:
                if no_show["user"].id not in user_ids:
                    raise ValidationError("user is not participant")
...
class ActivitySerializer(serializers.ModelSerializer):
    ...
    def validate_activity_type(self, activity_type):
        if activity_type.status != "active":
            raise ValidationError("You can only create activities for active types")
        return activity_type
    ...
    def validate_date(self, date):
        ...
        if duration < timedelta(seconds=1):
            raise ValidationError("Duration must be at least one second.")
        return date
        ...
        for participant_type in participant_types:
            ...
            if series_participant_type:
                if not self.instance:
                    ...
                    raise ValidationError("Cannot specify series_participant_type for new activity.")
                if series_participant_type.activity_series_id != self.instance.series_id:
                    raise ValidationError("Wrong series.")
    ...
    def validate(self, data):
        ...
        if activity_type and place and activity_type.group_id != place.group_id:
            raise ValidationError("ActivityType is not for this group.")
...
class ActivityUpdateSerializer(ActivitySerializer):
    ...
    def validate_date(self, date):
        if self.instance.series is not None and abs((self.instance.date.start - date.start).total_seconds()) > 1:
            raise ValidationError("You can't move activities that are part of a series.")
    ...
    def validate_has_duration(self, has_duration):
        if self.instance.series is not None and has_duration != self.instance.has_duration:
            raise ValidationError("You cannot modify the duration of activities that are part of a series")
        return has_duration
...
class ActivitySeriesSerializer(serializers.ModelSerializer):
    ...
    def validate_activity_type(self, activity_type):
        if activity_type.status != "active":
            raise ValidationError("You can only create series for active types")
        return activity_type
    ...
    def validate_rule(self, rule_string):
        ...
        if not isinstance(rrule, dateutil.rrule.rrule):
            raise ValidationError("Only single recurrence rules are allowed.")
        return rule_string
