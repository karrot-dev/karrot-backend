...
def process_finished_pickup_dates():
    ...
    stats_utils.periodic_task(
        "pickups__process_finished_pickup_dates", seconds=t.elapsed_seconds
    )
...
def daily_pickup_notifications():
    ...
    stats_utils.periodic_task(
        "pickups__daily_pickup_notifications", seconds=t.elapsed_seconds
    )
...
def fetch_pickup_notification_data_for_group(group):
    ...
    tomorrow = {
        "date__startswith__gte": midnight,
        "date__startswith__lt": midnight_tomorrow,
    }
    ...
    pickups = (
        PickupDate.objects.exclude_disabled()
        .annotate_num_collectors()
        .filter(place__status=PlaceStatus.ACTIVE.value, place__group=group,)
        .order_by("date")
    )
    ...
    ).exclude(groupmembership__user__in=User.objects.unverified(),)
    ...
    for user in users:
        ...
        base_tomorrow_not_full = subscribed_places_pickups.filter(
            **tomorrow, **not_full
        )
        ...
        user_pickups = PickupDate.objects.filter(
            place__group=group, collectors__in=[user],
        ).order_by("date")
