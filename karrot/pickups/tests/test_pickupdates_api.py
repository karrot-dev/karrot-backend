...
class TestPickupDatesAPI(APITestCase, ExtractPaginationMixin):
    ...
    def setUpTestData(cls):
        ...
        cls.past_pickup = PickupDateFactory(
            place=cls.place, date=to_range(timezone.now() - relativedelta(days=1))
        )
    ...
    def test_patch_past_pickup_fails(self):
        ...
        response = self.client.patch(
            self.past_pickup_url, self.pickup_data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN, response.data)
    ...
    def test_patch_as_newcomer_fails(self):
        ...
        self.client.force_login(user=newcomer)
        response = self.client.patch(
            self.pickup_url, {"max_collectors": 1}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN, response.data)
    ...
    def test_leave_pickup_as_member(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["notifications"], ConversationNotificationStatus.NONE.value
        )
    ...
    def test_get_conversation_as_collector(self):
        ...
        response = self.client.post(
            "/api/messages/", {"conversation": response.data["id"], "content": "hey",}
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, response.data)
    ...
    def test_cannot_get_conversation_as_nonmember(self):
        ...
        conversation_id = self.pickup.conversation.id
        response = self.client.post(
            "/api/messages/", {"conversation": conversation_id, "content": "hey",}
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN, response.data)
    ...
    def test_patch_date(self):
        ...
        response = self.client.patch(
            self.pickup_url,
            {"date": [start, end], "has_duration": True,},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
    ...
    def test_remove_duration_resets_end_date(self):
        ...
        response = self.client.patch(
            self.pickup_url, {"date": [start, end], "has_duration": True}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        response = self.client.patch(
            self.pickup_url, {"has_duration": False}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        ...
        self.assertEqual(
            self.pickup.date,
            CustomDateTimeTZRange(start, start + timedelta(minutes=30)),
        )
    ...
    def test_cannot_set_empty_duration(self):
        ...
        response = self.client.patch(
            self.pickup_url,
            {"date": [start, start], "has_duration": True},
            format="json",
        )
        self.assertEqual(
            response.status_code, status.HTTP_400_BAD_REQUEST, response.data
    ...
    def test_patch_start_date_only_uses_default_duration(self):
        self.client.force_login(user=self.member)
        start = timezone.now() + timedelta(hours=1)
        response = self.client.patch(self.pickup_url, {"date": [start],}, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.pickup.refresh_from_db()
    ...
    def test_patch_date_with_single_date_fails(self):
        self.client.force_login(user=self.member)
        response = self.client.patch(
            self.pickup_url, {"date": timezone.now(),}, format="json"
        )
        self.assertEqual(
            response.status_code, status.HTTP_400_BAD_REQUEST, response.data
        )
    ...
    def test_patch_end_date_only_fails(self):
        self.client.force_login(user=self.member)
        response = self.client.patch(
            self.pickup_url, {"date": [None, timezone.now()],}, format="json"
        )
        self.assertEqual(
            response.status_code, status.HTTP_400_BAD_REQUEST, response.data
        )
