...
class FeedbackTest(APITestCase, ExtractPaginationMixin):
    ...
    def setUpTestData(cls):
        ...
        cls.old_pickup = PickupDateFactory(
            place=cls.place,
            date=to_range(
                timezone.now() - relativedelta(days=settings.FEEDBACK_POSSIBLE_DAYS + 2)
            ),
            collectors=[cls.collector3,],
        )
        cls.old_feedback = FeedbackFactory(
            about=cls.old_pickup, given_by=cls.collector3
        )
    ...
    def test_create_feedback_fails_as_non_group_member(self):
        ...
        self.assertEqual(
            response.data, {"about": ["You are not member of the place's group."]}
        )
    ...
    def test_create_feedback_fails_as_non_collector(self):
        ...
        self.assertEqual(
            response.data, {"about": ["You aren't assigned to the pickup."]}
        )
    ...
    def test_create_feedback_fails_for_old_pickup(self):
        ...
        response = self.client.post(
            self.url, self.feedback_for_old_pickup, format="json"
        )
        self.assertEqual(
            response.status_code, status.HTTP_400_BAD_REQUEST, response.data
        )
    ...
    def test_create_feedback_without_weight(self):
        ...
        self.client.force_login(user=self.collector3)
        response = self.client.post(
            self.url, {k: v for (k, v) in self.feedback_post.items() if k != "weight"}
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, response.data)
    ...
    def test_create_feedback_without_comment(self):
        ...
        self.client.force_login(user=self.collector3)
        response = self.client.post(
            self.url, {k: v for (k, v) in self.feedback_post.items() if k != "comment"}
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, response.data)
    ...
    def test_weight_and_comment_is_null_fails(self):
        ...
        self.client.force_login(user=self.collector3)
        response = self.client.post(
            self.url, self.feedback_without_weight_comment, format="json"
        )
        self.assertEqual(
            response.status_code, status.HTTP_400_BAD_REQUEST, response.data
        )
    ...
    def test_patch_feedback_fails_as_non_user(self):
        ...
        response = self.client.patch(
            self.feedback_url, self.feedback_post, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN, response.data)
    ...
    def test_patch_feedback_fails_if_pickup_too_old(self):
        self.client.force_login(user=self.collector3)
        response = self.client.patch(
            self.old_feedback_url, {"weight": 499}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN, response.data)
        self.assertEqual(
    ...
    def test_patch_feedback_to_remove_comment_and_weight_fails(self):
        self.client.force_login(user=self.collector)
        response = self.client.patch(
            self.feedback_url, {"comment": "", "weight": None}, format="json"
        )
        self.assertEqual(
            response.status_code, status.HTTP_400_BAD_REQUEST, response.data
        )
