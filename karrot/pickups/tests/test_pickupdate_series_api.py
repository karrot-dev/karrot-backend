...
from karrot.pickups.factories import PickupDateSeriesFactory
...
class TestPickupDateSeriesCreationAPI(APITestCase, ExtractPaginationMixin):
    ...
    def test_create_and_get_recurring_series(self):
        ...
        recurrence = rrule.rrule(
            freq=rrule.WEEKLY, byweekday=[0, 1]
        )  # Monday and Tuesday
        start_date = self.group.timezone.localize(
        ...
        dates_list = recurrence.replace(
            ...
            .astimezone(tz)
            .replace(hour=20, minute=0, second=0, microsecond=0, tzinfo=None)
        ).between(
        ...
        for response_data_item, expected_date in zip(response.data, dates_list):
            self.assertEqual(
                parse(response_data_item["date"][0]),
                expected_date,
...
class TestPickupDateSeriesChangeAPI(APITestCase, ExtractPaginationMixin):
    ...
    def test_change_start_time(self):
        ...
        for response_pickup, old_date in zip(response.data, original_dates):
            self.assertEqual(
                ...
                shift_date_in_local_time(
                    old_date, relativedelta(hours=2, minutes=20), self.group.timezone
                ),
            )
    ...
    def test_change_start_date_to_future(self):
        ...
        url = "/api/pickup-date-series/{}/".format(self.series.id)
        new_startdate = shift_date_in_local_time(
            self.series.start_date, relativedelta(days=5), self.group.timezone
        )
        response = self.client.patch(url, {"start_date": new_startdate.isoformat()})
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(parse(response.data["start_date"]), new_startdate)
        ...
        for response_pickup, old_date in zip_longest(response.data, original_dates):
            self.assertEqual(
                parse(response_pickup["date"][0]),
                shift_date_in_local_time(
                    old_date, relativedelta(days=5), self.group.timezone
                ),
            )
    ...
    def test_change_start_date_to_past(self):
        ...
        for response_pickup, old_date in zip_longest(response.data, original_dates):
            new_date = shift_date_in_local_time(
                old_date, relativedelta(days=2), self.group.timezone
            )
    ...
    def test_set_end_date(self):
        ...
        # change rule
        url = "/api/pickup-date-series/{}/".format(self.series.id)
        rule = "FREQ=WEEKLY;UNTIL={}".format(
            (self.now + relativedelta(days=8)).strftime("%Y%m%dT%H%M%S")
        )
    ...
    def test_set_end_date_with_timezone(self):
        self.client.force_login(user=self.member)
        url = "/api/pickup-date-series/{}/".format(self.series.id)
        rule = "FREQ=WEEKLY;UNTIL={}+0100".format(
            (self.now + relativedelta(days=8)).strftime("%Y%m%dT%H%M%S")
        )
        response = self.client.patch(url, {"rule": rule})
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
    ...
    def test_set_multiple_rules_fails(self):
        ...
        self.assertEqual(
            response.data, {"rule": ["Only single recurrence rules are allowed."]}
        )
    ...
    def test_keep_changes_to_description(self):
        ...
        series_url = "/api/pickup-date-series/{}/".format(self.series.id)
        response = self.client.patch(
            series_url, {"description": "new series description"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
    ...
    def test_keeps_joined_pickups(self):
        ...
        tomorrow = (
            shift_date_in_local_time(
                self.now, relativedelta(days=1), self.group.timezone
            )
            .astimezone(self.group.timezone)
            .weekday()
        )
        recurrence = rrule.rrule(freq=rrule.WEEKLY, byweekday=[today, tomorrow,],)
        series_url = "/api/pickup-date-series/{}/".format(self.series.id)
        self.client.force_login(user=self.member)
        response = self.client.patch(series_url, {"rule": str(recurrence),})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.series.refresh_from_db()

        response = self.client.get(
            "/api/pickup-dates/?series={}".format(self.series.id)
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.assertEqual(
            ...
            list(
                interleave(
                    [[self.member.id] for _ in range(4)], [[] for _ in range(4)],
                )
            ),
        )
    ...
    def test_removes_empty_leftover_pickups_when_reducing_weeks_in_advance(self):
        ...
        self.client.force_login(user=self.member)
        response = self.client.patch(place_url, {"weeks_in_advance": 1,})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
