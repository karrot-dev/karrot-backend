...
from karrot.pickups.models import PickupDate as PickupDateModel, to_range
...
class TestPickupdatesAPIFilter(APITestCase, ExtractPaginationMixin):
    ...
    def test_filter_by_group(self):
        ...
        self.assertEqual(
            len(response.data),
            sum([place.pickup_dates.count() for place in self.group.places.all()]),
        )
    ...
    def test_filter_after_date(self):
        ...
        selected_pickups = PickupDateModel.objects.filter(
            place__group__members=self.member
...
class TestFeedbackPossibleFilter(APITestCase, ExtractPaginationMixin):
    def setUp(self):
        ...
        self.tooLongAgo = to_range(
            timezone.now() - relativedelta(days=settings.FEEDBACK_POSSIBLE_DAYS + 1),
            minutes=30,
        )
        ...
        self.pickupFeedbackPossible = PickupDateFactory(
            place=self.place, collectors=[self.member,], date=self.oneWeekAgo
        )
        ...
        self.pickupNotCollector = PickupDateFactory(
            place=self.place, date=self.oneWeekAgo
        )
        self.pickupTooLongAgo = PickupDateFactory(
            place=self.place, date=self.tooLongAgo
        )
        ...
        self.feedback = FeedbackFactory(
            about=self.pickupFeedbackAlreadyGiven, given_by=self.member
        )

        self.pickupCollectorLeftGroup = PickupDateFactory(
            place=self.place2, collectors=[self.member,], date=self.oneWeekAgo
        )
        self.pickupDoneByAnotherUser = PickupDateFactory(
            place=self.place, collectors=[self.member2,], date=self.oneWeekAgo
        )
