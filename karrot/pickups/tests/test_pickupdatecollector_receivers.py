...
class TestPickUpCollectorReceivers(APITestCase):
    def setUp(self):
        ...
        self.pickup = PickupDateFactory(
            place=self.place, collectors=[self.first_member]
        )

    def test_new_collector_marks_existing_messages_as_read(self):
        self.pickup.conversation.messages.create(
            author=self.first_member, content="foo"
        )
        self.pickup.conversation.messages.create(
            author=self.first_member, content="bar"
        )

        PickupDateCollector.objects.create(
            user=self.second_member, pickupdate=self.pickup
        )
        ...
        self.assertTrue(
            new_participant.seen_up_to == self.pickup.conversation.latest_message
        )
    ...
    def test_new_collector_does_not_remove_conversation_subscribers(self):
        ...
        PickupDateCollector.objects.create(
            user=self.third_member, pickupdate=self.pickup
        )
        self.assertIn(self.second_member, self.pickup.conversation.participants.all())
    ...
    def test_collector_leaving_does_not_remove_conversation_subscribers(self):
        ...
        PickupDateCollector.objects.filter(
            user=self.first_member, pickupdate=self.pickup
        ).delete()
        self.assertIn(self.second_member, self.pickup.conversation.participants.all())
