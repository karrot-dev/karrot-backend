...
from karrot.pickups.utils import (
    match_pickups_with_dates,
    rrule_between_dates_in_local_time,
)
...
class PickupDateSeries(BaseModel):
    ...
    place = models.ForeignKey(
        "places.Place", related_name="series", on_delete=models.CASCADE
    )
        ...
        if old:
            ...
            if description_changed or max_collectors_changed or duration_changed:
                for pickup in self.pickup_dates.upcoming():
                    ...
                    if (
                        max_collectors_changed
                        and old.max_collectors == pickup.max_collectors
                    ):
                    ...
                    if duration_changed:
                        if self.duration:
                            ...
                            pickup.date = CustomDateTimeTZRange(
                                pickup.date.start, pickup.date.start + self.duration
                            )
                        else:
    ...
    def delete(self, **kwargs):
        self.rule = str(
            rrulestr(self.rule).replace(dtstart=self.start_date, until=timezone.now())
        )
        self.update_pickups()
        return super().delete()
...
class PickupDateQuerySet(models.QuerySet):
    def _feedback_possible_q(self, user):
        return (
            Q(is_done=True)
            & Q(
                date__endswith__gte=timezone.now()
                - relativedelta(days=settings.FEEDBACK_POSSIBLE_DAYS)
            )
            & Q(collectors=user)
    ...
    def due_soon(self):
        in_some_hours = timezone.now() + relativedelta(
            hours=settings.PICKUPDATE_DUE_SOON_HOURS
        )
        return self.exclude_disabled().filter(
            date__startswith__gt=timezone.now(), date__startswith__lt=in_some_hours
        )

    def missed(self):
        return self.exclude_disabled().filter(
            date__startswith__lt=timezone.now(), collectors=None
        )

    def done(self):
        return (
            self.exclude_disabled()
            .filter(date__startswith__lt=timezone.now())
            .exclude(collectors=None)
        )
    ...
    def process_finished_pickup_dates(self):
        ...
        for pickup in self.exclude_disabled().filter(
            is_done=False, date__startswith__lt=timezone.now(),
        ):
...
class PickupDate(BaseModel, ConversationMixin):
    ...
    place = models.ForeignKey(
        "places.Place", related_name="pickup_dates", on_delete=models.CASCADE,
    )
    ...
    def is_recent(self):
        return self.date.start >= timezone.now() - relativedelta(
            days=settings.FEEDBACK_POSSIBLE_DAYS
        )
    ...
    def add_collector(self, user):
        collector, _ = PickupDateCollector.objects.get_or_create(
            pickupdate=self, user=user,
        )
        return collector

    def remove_collector(self, user):
        PickupDateCollector.objects.filter(pickupdate=self, user=user,).delete()
...
class PickupDateCollector(BaseModel):
    pickupdate = models.ForeignKey(PickupDate, on_delete=models.CASCADE,)
