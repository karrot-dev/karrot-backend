...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        def rename_columns(key_from, key_to):
            ...
            """
            for table_name, _column_name in fetchall(fkey_query):
        ...
        def rename_constraints(constraint_from, constraint_to):
            ...
            for table_name, constraint_name in fetchall(constraints_query):
                return (
                    f"alter table {table_name} rename constraint {constraint_name} to "
                    f"{constraint_name.replace(constraint_from, constraint_to)}"
                )
