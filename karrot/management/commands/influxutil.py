...
class Command(BaseCommand):
    def add_arguments(self, parser):
        parser.add_argument(
            "values", metavar="arg", type=str, nargs="+", help="params for the commands"
        )
    ...
    def handle(self, *args, **options):
        ...
        if command == "export":
            ...
            sed = "; ".join(
                [
                    "s/store/place/g",
                    's/CREATE DATABASE.*WITH/CREATE DATABASE "{}" WITH/'.format(
                        to_db_name
                    ),
            ...
            execute(
                "zcat {} | sed {} | gzip > {}".format(
                    quote(filename), quote(sed), quote(filename + ".rewritten")
                ),
                shell=True,
            )
