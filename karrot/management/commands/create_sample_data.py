...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        def teardown_environment():
            ...
            settings.INFLUXDB_DISABLED = (
                request._BLAH_original_influxdb_disable
            ) = settings.INFLUXDB_DISABLED
        ...
        def apply_to_group(group):
            response = c.post(
                "/api/applications/", {"answers": faker.text(), "group": group,}
            )
            if response.status_code != 201:
        ...
        def make_message(conversation_id):
            response = c.post(
                "/api/messages/",
                {"content": faker.text(), "conversation": conversation_id,},
            )
            if response.status_code != 201:
                raise Exception(
                    "could not make message", conversation_id, response.data
                )
            return response.data
        ...
        def make_pickup(place):
            date = to_range(
                faker.date_time_between(
                    start_date="+2d", end_date="+7d", tzinfo=pytz.utc
                )
            )
            response = c.post(
                "/api/pickup-dates/",
                {"date": date.as_list(), "place": place, "max_collectors": 10},
                format="json",
            )
            if response.status_code != 201:
        ...
        def modify_pickup(pickup):
            response = c.patch(
                "/api/pickup-dates/{}/".format(pickup), {"max_collectors": 3}
            )
            if response.status_code != 200:
        ...
        def create_done_pickup(place, user_id):
            pickup = PickupDate.objects.create(
                date=to_range(
                    faker.date_time_between(
                        start_date="-9d", end_date="-1d", tzinfo=pytz.utc
                    ),
                    minutes=30,
                ),
                place_id=place,
        ...
        for g in groups:
            two_users = User.objects.filter(groupmembership__group=g["id"]).distinct()[
                :2
            ]
        ...
        u = login_user()
        o = PickupDateSeries.objects.filter(
        ...
        u = login_user()
        o = PickupDate.objects.filter(
            date__startswith__gte=timezone.now() + relativedelta(minutes=10),
            collectors=u,
        ).first()
        leave_pickup(o.id)
        ...
        for _ in range(n_done):
            u = login_user()
            p = (
                PickupDate.objects.filter(
                    date__startswith__gte=timezone.now() + relativedelta(hours=1),
                    place__group__members=u,
                )
                .exclude(collectors=u)
                .first()
            )
            join_pickup(p.id)
        ...
        print_success(
            "Done! You can login with any of those mail addresses and password {}".format(
                default_password
            )
        )
        if not options["more_data"]:
            print_success(
                "Consider using the --more argument next time for more users."
            )
