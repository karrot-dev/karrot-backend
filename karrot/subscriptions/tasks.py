...
def notify_message_push_subscribers(message):
    ...
    subscriptions = (
        subscriptions.exclude(user=message.author)
        .select_related("user")
        .order_by("user__language")
        .distinct()
    )

    for (language, subscriptions) in groupby(
        subscriptions, key=lambda subscription: subscription.user.language
    ):
...
def notify_new_offer_push_subscribers(offer):
    ...
    subscriptions = (
        PushSubscription.objects.filter(user__in=users,)
...
def notify_subscribers_by_device(subscriptions, *, click_action, fcm_options):
    android_subscriptions = [
        s for s in subscriptions if s.platform == PushSubscriptionPlatform.ANDROID.value
    ]
    web_subscriptions = [
        s for s in subscriptions if s.platform == PushSubscriptionPlatform.WEB.value
    ]
