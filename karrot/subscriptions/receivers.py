...
def send_group_preview(group):
    for subscription in ChannelSubscription.objects.recent():
        preview_payload = GroupPreviewSerializer(
            group,
            context={"request": MockRequest(user=subscription.user, META={"REMOTE_ADDR": subscription.client_ip})},
        ).data
...
def send_group_member_left(sender, instance, **kwargs):
    ...
    for subscription in ChannelSubscription.objects.recent().filter(
        Q(user__in=group.members.all()) | Q(user_id=membership.user_id)
    ):
        send_in_channel(
            subscription.reply_channel,
...
def send_invitation_updates(sender, instance, **kwargs):
    ...
    for subscription in ChannelSubscription.objects.recent().filter(user__in=invitation.group.members.all()).distinct():
...
def send_activity_updates(sender, instance, **kwargs):
    ...
    for subscription in (
        ChannelSubscription.objects.recent().filter(user__in=activity.place.group.members.all()).distinct()
    ):
...
def send_activity_series_updates(sender, instance, **kwargs):
    ...
    for subscription in (
        ChannelSubscription.objects.recent().filter(user__in=series.place.group.members.all()).distinct()
    ):
...
def send_activity_type_updates(sender, instance, **kwargs):
    ...
    for subscription in (
        ChannelSubscription.objects.recent().filter(user__in=activity_type.group.members.all()).distinct()
    ):
...
def send_offer_updates(sender, instance, created, **kwargs):
    ...
    for subscription in ChannelSubscription.objects.recent().filter(user__in=offer.group.members.all()).distinct():
        if (
            not offer.is_archived
            or offer.user == subscription.user
            or subscription.user.conversation_set.filter(id=offer.conversation.id).exists()
        ):
...
def send_feedback_updates(sender, instance, **kwargs):
    feedback = instance
    for subscription in (
        ChannelSubscription.objects.recent().filter(user__in=feedback.about.place.group.members.all()).distinct()
    ):
...
def send_user_updates(sender, instance, **kwargs):
    ...
    for subscription in (
        ChannelSubscription.objects.recent().filter(user__groups__in=user_groups).exclude(user=user).distinct()
    ):
...
def send_issue_updates(sender, issue, **kwargs):
    for subscription in (
        ChannelSubscription.objects.recent().filter(user__groupmembership__group=issue.group).distinct()
    ):
