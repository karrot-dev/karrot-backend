...
from karrot.subscriptions.tasks import (
    get_message_title,
    notify_message_push_subscribers,
)
...
class TestMessagePushNotifications(TestCase):
    def test_message_notification(self, notify):
        ...
        subscriptions = [
            PushSubscription.objects.create(
                user=user, token="", platform=PushSubscriptionPlatform.ANDROID.value
            )
        ]
    ...
    def test_groups_by_language(self, notify):
        author = UserFactory()
        users = [
            UserFactory(language=language) for language in ("de", "de", "en", "fr")
        ]
        ...
        subscriptions = [
            PushSubscription.objects.create(
                user=u, token="", platform=PushSubscriptionPlatform.ANDROID.value
            )
            for u in users
        ]
    ...
    def test_no_reply_notification_if_muted(self, notify):
        ...
        participant = ConversationThreadParticipant.objects.get(
            user=author, thread=reply.thread
        )
        participant.muted = True
        participant.save()
        PushSubscription.objects.create(
...
class TestMessagePushNotificationTitles(TestCase):
    ...
    def test_pickup_message_title(self):
        ...
        pickup = PickupDateFactory(
            place=place,
            collectors=[author],
            date=to_range(parse("2018-11-11T20:00:00Z")),
        )
    ...
    def test_application_message_title(self):
        ...
        title = get_message_title(message, "en")
        self.assertEqual(
            title, "❓ {} / {}".format(applicant.display_name, author.display_name)
        )
        ...
        title = get_message_title(message, "en")
        self.assertEqual(
            title, "🗑️ {} / {}".format(applicant.display_name, author.display_name)
        )
