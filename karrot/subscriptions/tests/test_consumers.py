...
from karrot.subscriptions.consumers import (
    WebsocketConsumer,
    TokenAuthMiddleware,
    get_auth_token_from_subprotocols,
)
...
class TokenCommunicator:
    async def __aenter__(self):
        ...
        self.communicator = WebsocketCommunicator(
            ...
            subprotocols=[
                "karrot.token",
                "karrot.token.value.{}".format(encoded.rstrip("=")),
            ],
        )
...
class TestConsumer:
    async def test_adds_subscription(self):  # noqa: F811
        async with Communicator() as communicator:
            ...
            assert (
                await get_subscription_count(user=user)
            ) == 1, "Did not add subscription"
    ...
    async def test_saves_reply_channel(self):
        async with Communicator() as communicator:
            ...
            await get_channel_layer().send(
                ...
                {
                    "type": "message.send",
                    "text": json.dumps({"message": "hey! whaatsup?",}),
                },
            )
...
class TestAllowedOriginValidator:
    async def test_can_connect_with_file_origin(self):
        ...
        communicator = WebsocketCommunicator(
            application, "/", headers=[(b"origin", b"file:///")]
        )
...
class TestTokenUtil:
    def test_get_auth_token_from_subprotocols(self):
        token = get_auth_token_from_subprotocols(
            ["karrot.token", "karrot.token.value.Zm9v",]
        )
        assert token == "foo"

    def test_get_auth_token_from_headers_with_removed_base64_padding(self):
        token = get_auth_token_from_subprotocols(
            ["karrot.token", "karrot.token.value.Zm9vMQ",]
        )
        assert token == "foo1"
