...
from karrot.subscriptions.models import (
    ChannelSubscription,
    PushSubscription,
...
class WSClient:
    ...
    def call_args(self):
        ...
        return [
            normalize_call_args(*args, **kwargs)
            for args, kwargs in self.send_in_channel_mock.call_args_list
        ]
    ...
    def messages_by_topic(self):
        ...
        return {
            topic: list(group)
            for (topic, group) in itertools.groupby(
                sorted(self.messages, key=topic), key=topic
            )
        }
...
class WSTestCase(TestCase):
    def setUp(self):
        super().setUp()
        self.send_in_channel_patcher = patch(
            "karrot.subscriptions.receivers.send_in_channel"
        )
...
class ConversationReceiverTests(WSTestCase):
    def test_receives_messages(self):
        ...
        self.assertEqual(
            len(ws_messages["conversations:message"]),
            1,
        ...
        parse_dates(response)
        self.assertEqual(
            response, make_conversation_message_broadcast(message, is_editable=True)
        )
    ...
    def tests_receive_message_on_leave(self):
        ...
        self.assertEqual(
            len(messages["conversations:leave"]), 1, messages["conversations:leave"]
        )
        self.assertEqual(
            messages["conversations:leave"][0]["payload"], {"id": conversation.id}
        )
    ...
    def test_other_participants_receive_update_on_join(self):
        ...
        self.assertEqual(
            set(response["payload"]["participants"]), {user.id, joining_user.id}
        )
...
class ConversationThreadReceiverTests(WSTestCase):
    def test_receives_messages(self):
        ...
        parse_dates(response)
        self.assertEqual(
            response, make_conversation_message_broadcast(reply, thread=thread.id,)
        )
        ...
        self.assertEqual(
            response,
            make_conversation_message_broadcast(
                reply, is_editable=True, thread=thread.id
            ),
        )
    ...
    def test_thread_marked_as_seen(self):
        ...
        thread = ConversationMessage.objects.create(
            conversation=conversation, content="yay", author=op_author
        )
        reply = ConversationMessage.objects.create(
            conversation=conversation,
            thread=thread,
...
class ConversationMessageReactionReceiverTests(WSTestCase):
    def test_receive_reaction_update(self):
        ...
        ConversationMessageReaction.objects.create(
            message=message, user=reaction_user, name="carrot",
        )
        ...
        self.assertEqual(
            response,
            make_conversation_message_broadcast(
                message, reactions=[{"name": "carrot", "user": reaction_user.id}]
            ),
        )
...
class GroupMembershipReceiverTests(WSTestCase):
    ...
    def test_receive_group_join(self):
        ...
        self.assertNotIn(
            "groups:group_detail", nonmember_client.messages_by_topic.keys()
        )
    ...
    def test_receive_group_roles_update(self):
        ...
        self.assertEqual(
            [m["topic"] for m in client.messages],
            ["notifications:notification", "status", "groups:group_detail",],
        )
...
class ApplicationReceiverTests(WSTestCase):
    ...
    def test_member_receives_application_create(self):
        ...
        self.assertEqual(
            messages[0]["payload"],
            {"groups": {self.group.id: {"pending_application_count": 1}}},
        )
    ...
    def test_member_receives_application_update(self):
        ...
        self.assertEqual(
            messages[1]["payload"],
            {"groups": {self.group.id: {"pending_application_count": 0}}},
        )
...
class InvitationReceiverTests(WSTestCase):
    ...
    def test_receive_invitation_updates(self):
        ...
        invitation = Invitation.objects.create(
            email="bla@bla.com", group=self.group, invited_by=self.member
        )
    ...
    def test_receive_invitation_accept(self):
        ...
        response = next(
            r for r in client.messages if r["topic"] == "invitations:invitation_accept"
        )
...
class PickupDateReceiverTests(WSTestCase):
    ...
    def test_mark_as_done(self):
        ...
        self.assertEqual(
            messages["status"][0]["payload"], {"unseen_notification_count": 1}
        )
        self.assertEqual(
            messages["status"][1]["payload"],
            {"groups": {self.group.id: {"feedback_possible_count": 1}}},
        )
        self.assertEqual(
            len(messages["notifications:notification"]),
            1,
...
class PickupDateSeriesReceiverTests(WSTestCase):
    def setUp(self):
        ...
        self.series = PickupDateSeriesFactory(
            place=self.place, start_date=timezone.now() + relativedelta(months=2)
        )
    ...
    def test_receive_series_changes(self):
        ...
        date = faker.future_datetime(
            end_date="+30d", tzinfo=timezone.utc
...
class FinishedPickupReceiverTest(WSTestCase):
    ...
    def test_receive_history_and_notification(self):
        ...
        self.assertEqual(
            status_messages[0]["payload"], {"unseen_notification_count": 1}
        )
        self.assertEqual(
...
class UserReceiverTest(WSTestCase):
    ...
    def test_receive_own_user_changes(self):
        ...
        self.assertTrue(
            response["payload"]["photo_urls"]["full_size"].startswith(settings.HOSTNAME)
        )
...
class ReceiverPushTests(TestCase):
    ...
    def test_sends_to_push_subscribers(self, notify_subscribers):
        ...
        self.assertEqual(
            kwargs["fcm_options"]["message_title"], self.author.display_name
        )
        self.assertEqual(kwargs["fcm_options"]["message_body"], self.content)

    def test_send_push_notification_if_active_channel_subscription(
    ...
    def test_send_push_notification_if_channel_subscription_is_away(
        self, notify_subscribers
    ):
        # add a channel subscription to prevent the push being sent
        ChannelSubscription.objects.create(
            user=self.user, reply_channel="foo", away_at=timezone.now()
        )
