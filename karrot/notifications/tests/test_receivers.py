...
class TestNotificationReceivers(TestCase):
    ...
    def test_creates_user_became_editor(self):
        ...
        user_became_editor = Notification.objects.get(
    ...
    def test_creates_new_applicant_notification(self):
        ...
        self.assertEqual(
            Notification.objects.filter(
                user=member, type=NotificationType.NEW_APPLICANT.value
            ).count(),
            0,
        )
    ...
    def test_creates_application_accepted_notification(self):
        ...
        self.assertEqual(
            Notification.objects.filter(
                user=user, type=NotificationType.APPLICATION_ACCEPTED.value
            ).count(),
            1,
        )
        self.assertEqual(
    ...
    def test_creates_application_declined_notification(self):
        ...
        self.assertEqual(
            Notification.objects.filter(
                user=user, type=NotificationType.APPLICATION_DECLINED.value
            ).count(),
            1,
        )
        self.assertEqual(
    ...
    def test_creates_feedback_possible_notification(self):
        ...
        notification = Notification.objects.filter(
            user=member, type=NotificationType.FEEDBACK_POSSIBLE.value
        )
        self.assertEqual(notification.count(), 1)
        self.assertLessEqual(
            notification[0].expires_at,
            pickup.date.end + relativedelta(days=settings.FEEDBACK_POSSIBLE_DAYS),
        )
    ...
    def test_creates_new_invitation_accepted_notification(self):
        ...
        invitation = Invitation.objects.create(
            email="bla@bla.com", group=group, invited_by=member1
        )
        Notification.objects.all().delete()
    ...
    def test_creates_pickup_disabled_notification_and_deletes_pickup_upcoming_notification(
        ...
        pickup = PickupDateFactory(
            place=place, date=in_one_hour, collectors=[user1, user2]
        )
        Notification.objects.all().delete()
    ...
    def test_creates_pickup_enabled_notification(self):
        ...
        pickup_enabled_notifications = Notification.objects.filter(
    ...
    def test_conflict_resolution_notifications(self):
        ...
        self.assertEqual(
            notifications[1].context,
            {"issue": issue.id, "group": group.id, "user": user2.id},
        )
        self.assertEqual(
            notifications[0].type, NotificationType.CONFLICT_RESOLUTION_CREATED.value
        )
        ...
        self.assertEqual(
            notifications[0].type, NotificationType.CONFLICT_RESOLUTION_CONTINUED.value
        )
        self.assertEqual(
            notifications[1].type, NotificationType.CONFLICT_RESOLUTION_CONTINUED.value
        )
        self.assertEqual(
