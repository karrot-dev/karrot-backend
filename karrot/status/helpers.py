...
def unseen_notification_count(user):
    return Notification.objects.filter(
        user=user, created_at__gt=F("user__notificationmeta__marked_at")
    ).count()
...
def unread_conversations(user):
    threads = (
        ...
        .aggregate(
            ...
            unseen=Count(
                "id",
                filter=Q(
                    thread__latest_message__created_at__gt=F(
                        "user__conversationmeta__threads_marked_at"
                    )
                ),
            ),
        )
    ...
    participants = list(
        ...
        .annotate(
            is_unseen=Case(
                When(
                    conversation__latest_message__created_at__gt=F(
                        "user__conversationmeta__conversations_marked_at"
                    ),
                    then=True,
                ),
                default=False,
    ...
    return {
        ...
        "has_unread_conversations_or_threads": len(participants) + threads["unread"]
        > 0,
        "groups": groups,
        "places": places,
    }
    ...
    return (
        ...
        .annotate(
            pending_application_count=Count(
                "application",
                filter=Q(application__status=ApplicationStatus.PENDING.value),
            )
        )
    ...
    return (
        ...
        .annotate(
            feedback_possible=Count(
                ...
                filter=Q(
                    places__pickup_dates__in=PickupDate.objects.only_feedback_possible(
                        user
                    ),
