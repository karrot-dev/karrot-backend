...
class TestEmailUtils(TestCase):
    ...
    def test_raises_exception_on_redefined_context(self):
        ...
        with self.assertRaisesMessage(
            ...
            prepare_email(
                template="accountdelete_success", user=user, context={"user": user,},
            )

    def test_emailinvitation(self):
        invitation = Invitation.objects.create(
            email="bla@bla.com", group=self.group, invited_by=self.user
        )
    ...
    def test_signup(self):
        ...
        email = karrot.users.emails.prepare_signup_email(
            user=self.user, verification_code=verification_code,
        )
...
class TestRetry(TestCase):
    ...
    def test_success_at_second_try(self, mock_send):
        ...
        CustomAnymailMessage(
            subject="Hi", body="test", to=["test@example.com"], stats_category="test"
        ).send()
...
class TestJinjaFilters(TestCase):
    def test_time_filter_uses_timezone(self):
        ...
        datetime = timezone.now().replace(
            tzinfo=pytz.utc, hour=hour, minute=0, second=0, microsecond=0,
        )
    ...
    def test_date_filter_uses_timezone(self):
        ...
        with timezone.override(pytz.timezone("Europe/Berlin")), translation.override(
            "en"
        ):
            # ... is Monday in Berlin
