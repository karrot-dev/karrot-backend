...
class AuthUserSerializer(serializers.ModelSerializer):
    photo = VersatileImageFieldSerializer(
        sizes="user_profile", required=False, allow_null=True, write_only=True
    )
    photo_urls = VersatileImageFieldSerializer(
        sizes="user_profile", read_only=True, source="photo"
    )
    ...
    class Meta:
        ...
        extra_kwargs = {
            "display_name": {"min_length": 3, "validators": [prevent_reserved_names],},
            "password": {"write_only": True,},
            "description": {
                "trim_whitespace": False,
                "max_length": settings.DESCRIPTION_MAX_LENGTH,
            },
            "mail_verified": {"default": False,},
    ...
    def validate_email(self, email):
        ...
        if similar.exists():
            raise serializers.ValidationError(
                _("Similar e-mail exists: ") + similar.first().email
            )
        return email
...
class VerificationCodeSerializer(serializers.Serializer):
    ...
    def validate_code(self, code):
        try:
            matched_code = VerificationCode.objects.get(
                code=code, type=self.context["type"]
            )
