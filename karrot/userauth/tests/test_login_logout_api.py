...
class TestUserAuthAPI(APITestCase):
    ...
    def test_login_as_disabled_user_fails(self):
        data = {
            ...
            "password": self.disabled_user.display_name,
        }
        response = self.client.post(self.url, data, format="json")
        self.assertEqual(
            response.data["non_field_errors"],
    ...
    def test_status_not_logged_in(self):
        ...
        self.assertEqual(
            response.data["error_code"], "not_authenticated", response.data
        )
...
class TestTokenAuthAPI(APITestCase):
    ...
    def test_get_token(self):
        response = self.client.post(
            self.url, {"username": self.user.email, "password": self.user.display_name}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_use_token(self):
        ...
        response = self.client.get(
            "/api/auth/user/", **{"HTTP_AUTHORIZATION": "Token {}".format(token.key)}
        )
