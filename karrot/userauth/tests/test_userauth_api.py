...
class TestUsersAPI(APITestCase):
    ...
    def test_create_user(self):
        ...
        self.assertAlmostEqual(
            response.data["latitude"], float(self.user_data["latitude"])
        )
    ...
    def test_patch_too_long_description(self):
        ...
        response = self.client.patch(
            self.url,
            {"description": "ab" * settings.DESCRIPTION_MAX_LENGTH},
            format="json",
        )
...
class TestUserDeleteAPI(APITestCase):
    ...
    def test_request_deletion_twice_succeeds(self):
        ...
        codes = VerificationCode.objects.filter(
            user=self.user, type=VerificationCode.ACCOUNT_DELETE
        )
    ...
    def test_delete_succeeds(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertFalse(
            self.group.members.get_queryset().filter(id=self.user.id).exists()
        )
        self.assertFalse(
            self.pickupdate.collectors.get_queryset().filter(id=self.user.id).exists()
        )
        self.assertTrue(
            self.past_pickupdate.collectors.get_queryset()
        ...
        self.assertEqual(
            get_user_model().objects.filter(email=self.user.email).count(), 0
        )
        ...
        self.assertEqual(
            self.client.post(
                "/api/auth/",
                {"email": self.user.email, "password": self.user.display_name},
            ).status_code,
...
class TestCreateUserErrors(APITestCase):
    ...
    def test_create_user_with_similar_cased_email_fails(self):
        ...
        response = self.client.post(
            self.url, {**self.user_data, "email": "Fancy@example.com"}
        )
        self.assertEqual(
            response.status_code, status.HTTP_400_BAD_REQUEST, response.data
        )
        self.assertEqual(
            response.data["email"], ["Similar e-mail exists: fancy@example.com"]
        )

    def test_cannot_create_user_without_email(self):
        response = self.client.post(
            self.url, {**self.user_data, "email": ""}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        response = self.client.post(
            self.url, {**self.user_data, "email": None}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_cannot_create_user_with_seemingly_invalid_email(self):
        response = self.client.post(
            self.url, {**self.user_data, "email": "asdf"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class TestSetCurrentGroup(APITestCase):
    ...
    def test_set_current_group_succeeds(self):
        ...
        self.assertEqual(
            self.client.patch(
                "/api/auth/user/", {"current_group": self.group.id}
            ).status_code,
            status.HTTP_200_OK,
        )
    ...
    def test_set_current_group_as_non_member_fails(self):
        ...
        self.assertEqual(
            self.client.patch(
                "/api/auth/user/", {"current_group": self.unrelated_group.id}
            ).status_code,
            status.HTTP_400_BAD_REQUEST,
        )
...
class TestChangeEMail(APITestCase):
    ...
    def test_change_succeeds(self):
        ...
        response = self.client.put(
            self.url, {"password": self.password, "new_email": self.new_email}
        )
    ...
    def test_change_without_password_fails(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        response = self.client.put(
            self.url, {"password": "", "new_email": self.new_email}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(len(mail.outbox), 0)
    ...
    def test_change_with_wrong_password_fails(self):
        self.client.force_login(user=self.verified_user)
        response = self.client.put(
            self.url, {"password": "wrong", "new_email": self.new_email}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(len(mail.outbox), 0)
    ...
    def test_change_to_existing_mail_fails(self):
        self.client.force_login(user=self.verified_user)
        response = self.client.put(
            self.url, {"password": self.password, "new_email": self.another_user.email}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(len(mail.outbox), 0)
    ...
    def test_change_to_existing_similar_mail_fails(self):
        self.client.force_login(user=self.verified_user)
        similar_mail = (
            self.another_user.email[0].swapcase() + self.another_user.email[1:]
        )
        response = self.client.put(
            self.url, {"password": self.password, "new_email": similar_mail}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
    ...
    def test_restore_email_succeeds(self):
        ...
        response = self.client.put(
            self.url, {"password": self.password, "new_email": self.old_email}
        )
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(len(mail.outbox), 0)
        self.assertEqual(self.verified_user.email, self.old_email)
    ...
    def test_changing_sensitive_fields_is_forbidden(self):
        ...
        response = self.client.patch(
            self.url_patch, {"unverified_email": faker.email()}
        )
        self.assertEqual(
            response.data["unverified_email"], original["unverified_email"]
        )
...
class TestEMailVerification(APITestCase):
    ...
    def test_verify_mail_fails_if_already_verified(self):
        code = VerificationCode.objects.get(
            user=self.user, type=VerificationCode.EMAIL_VERIFICATION
        ).code
