...
class TestPasswordReset(APITestCase):
    ...
    def test_request_password_reset_succeeds(self):
        response = self.client.post(
            self.url_request_password_reset, {"email": self.verified_user.email}
        )
    ...
    def test_request_password_reset_fails_if_wrong_mail(self):
        response = self.client.post(
            self.url_request_password_reset, {"email": "wrong@example.com"}
        )
    ...
    def test_request_password_reset_with_similar_email_succeeds(self):
        response = self.client.post(
            self.url_request_password_reset, {"email": "RESET_test@example.com"}
        )
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(len(mail.outbox), 1)
        self.assertEqual(mail.outbox[0].to, [self.verified_user.email])
    ...
    def test_request_password_reset_twice_succeeds(self):
        ...
        code = VerificationCode.objects.get(
            user=self.verified_user, type=self.type
        ).code
        response = self.client.post(
            self.url_password_reset, {"code": code, "new_password": self.new_password}
        )
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
    ...
    def test_password_reset_succeeds(self):
        ...
        self.assertFalse(
            self.client.login(
                email=self.verified_user.email, password=self.old_password
            )
        )
        ...
        self.assertTrue(
            self.client.login(
                email=self.verified_user.email, password=self.new_password
            )
        )
    ...
    def test_password_reset_fails_without_verification_code(self):
        ...
        response = self.client.post(
            self.url_password_reset, {"new_password": self.new_password}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_password_reset_fails_if_verification_code_too_old(self):
        ...
        response = self.client.post(
            ...
            {"code": verification_code.code, "new_password": self.new_password},
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
