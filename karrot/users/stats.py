...
def get_users_stats():
    ...
    active_users = User.objects.filter(
        groupmembership__in=GroupMembership.objects.active(), deleted=False
    ).distinct()
    ...
    fields = {
        ...
        "active_with_location_count": active_users.exclude(latitude=None)
        .exclude(longitude=None)
        .count(),
        "active_with_mobile_number_count": active_users.exclude(
            mobile_number=""
        ).count(),
        ...
        "active_memberships_per_active_user_avg": active_membership_count
        / active_users_count,
        "no_membership_count": User.objects.filter(
            groupmembership=None, deleted=False
        ).count(),
    ...
    for n in (1, 7, 30, 60, 90):
        active_users = User.objects.filter(
            groupmembership__in=GroupMembership.objects.exclude_playgrounds().active_within(
                days=n
            ),
            deleted=False,
        ).distinct()
        ...
        pickup_active_users = User.objects.filter(
            pickup_dates__in=PickupDate.objects.exclude_disabled()
            .filter(
