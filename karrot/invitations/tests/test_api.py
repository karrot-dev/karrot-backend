...
class TestInvitationAPIIntegration(APITestCase):
    ...
    def test_invite_flow(self):
        ...
        response = self.client.post(
            base_url, {"email": "please@join.com", "group": self.group.id}
        )
...
class TestInviteCreate(APITestCase):
    def setUp(self):
        ...
        self.group = GroupFactory(
            members=[self.member, self.member2], newcomers=[self.newcomer]
        )
    ...
    def test_invite_same_email_to_different_groups(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        response = self.client.post(
            base_url, {"email": "please@join.com", "group": self.group2.id}
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_invite_existing_user_but_not_member(self):
        self.client.force_login(self.member)
        response = self.client.post(
            base_url, {"email": self.member2.email, "group": self.group2.id}
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_invite_existing_member(self):
        self.client.force_login(self.member)
        response = self.client.post(
            base_url, {"email": self.member2.email, "group": self.group.id}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_invite_to_invalid_group(self):
        self.client.force_login(self.member)
        response = self.client.post(
            base_url, {"email": "please@join.com", "group": 345236}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_invite_with_invalid_email(self):
        self.client.force_login(self.member)
        response = self.client.post(
            base_url, {"email": "pleasehä", "group": self.group.id}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_invite_with_rejected_email(self, send):
        ...
        self.client.force_login(self.member)
        response = self.client.post(
            base_url, {"email": "rejected@foo.com", "group": self.group.id}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class TestInvitationAcceptAPI(APITestCase):
    ...
    def test_accept_invite_when_already_in_group(self):
        self.client.force_login(self.member)
        self.client.post(
            base_url, {"email": "someother@mail.com", "group": self.group.id}
        )
