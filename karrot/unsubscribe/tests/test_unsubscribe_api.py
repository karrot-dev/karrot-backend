...
class TestTokenUnsubscribeAPI(APITestCase):
    ...
    def test_unsubscribe_from_conversation(self):
        participant = self.group.conversation.conversationparticipant_set.filter(
            user=self.user
        )
        token = generate_token(
            self.user, self.group, conversation=self.group.conversation
        )
        self.assertFalse(participant.get().muted)
        self.client.post(
            self.url.format(token), {"choice": "conversation"}, format="json"
        )
        self.assertTrue(participant.get().muted)

    def test_unsubscribe_from_thread(self):
        thread = self.group.conversation.messages.create(
            author=self.user, content="foo"
        )
        self.group.conversation.messages.create(
            author=self.user, content="foo reply", thread=thread
        )
    ...
    def test_unsubscribe_from_notification_type(self):
        ...
        notification_types = self.group.groupmembership_set.filter(
            user=self.user
        ).values_list("notification_types", flat=True,)
        self.assertIn("new_application", notification_types.get())
        self.client.post(
            self.url.format(token), {"choice": "notification_type"}, format="json"
        )
    ...
    def test_fails_with_invalid_token(self):
        response = self.client.post(
            self.url.format("invalidtoken"), {"choice": "group"}, format="json"
        )
        self.assertEqual(
    ...
    def test_fails_without_a_thread(self):
        ...
        response = self.client.post(
            self.url.format(token), {"choice": "thread"}, format="json"
        )
        self.assertEqual(
            response.status_code, status.HTTP_400_BAD_REQUEST, response.data
        )
    ...
    def test_fails_without_a_group(self):
        ...
        response = self.client.post(
            self.url.format(token), {"choice": "group"}, format="json"
        )
        self.assertEqual(
            response.status_code, status.HTTP_400_BAD_REQUEST, response.data
        )
...
class TestUnsubscribeAPI(APITestCase):
    ...
    def test_unsubscribe_from_group(self):
        ...
        self.assertFalse(participant.get().muted)
        response = self.client.post(
            self.url, {"choice": "group", "group": self.group.id}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data, {"conversations": 1, "threads": 0,})
        self.assertTrue(participant.get().muted)
