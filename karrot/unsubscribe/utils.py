...
def parse_token(token):
    ...
    if "t" in data:
        result.update(
            {
                "thread": ConversationMessage.objects.only_threads_with_user(user).get(
                    pk=data["t"]
                )
            }
        )
...
def unsubscribe_from_thread(user, thread):
    p = ConversationThreadParticipant.objects.filter(user=user, thread=thread,).first()
    if p:
        p.muted = True
        p.save()
...
def unsubscribe_from_all_conversations_in_group(user, group):
    ...
    for p in ConversationParticipant.objects.filter(
        user=user, conversation__in=conversations, muted=False,
    ):
        p.muted = True
        p.save()
    ...
    for p in ConversationThreadParticipant.objects.filter(
        user=user, thread__conversation__in=conversations, muted=False,
    ):
        p.muted = True
        p.save()
        thread_count += 1
