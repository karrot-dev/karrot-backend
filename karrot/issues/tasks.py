...
def process_expired_votings():
    ...
    stats_utils.periodic_task(
        "issues__process_expired_votings", seconds=t.elapsed_seconds
    )


def get_users_to_notify(issue):
    return (
        issue.participants.filter(
            groupmembership__notification_types__contains=[
...
def notify_about_new_conflict_resolution(issue):
    ...
    for user in (
        get_users_to_notify(issue)
        .exclude(id=issue.created_by_id)
        .exclude(id=issue.affected_user_id)
    ):
...
def notify_about_continued_conflict_resolution(issue):
    send_or_report_error(
        prepare_conflict_resolution_continued_email_to_affected_user(issue)
    )
