...
class VoteListSerializer(serializers.ListSerializer):
    def validate(self, attrs):
        ...
        if not all((option.id in mapping) for option in voting.options.all()):
            raise serializers.ValidationError(
                _("You need to provide a score for all options")
            )
    ...
    def create(self, validated_data):
        return self.context["voting"].save_votes(
            user=self.context["request"].user, vote_data=validated_data,
        )
...
class VoteSerializer(serializers.ModelSerializer):
    ...
    def validate_option(self, option):
        ...
        if option.voting != voting:
            raise serializers.ValidationError(
                _("Provided option is not part of this voting")
            )
        return option
    ...
    def validate_score(self, score):
        if not -2 <= score <= 2:
            raise serializers.ValidationError(
                _("Provided score is outside of allowed range")
            )
        return score
...
class IssueSerializer(serializers.ModelSerializer):
    ...
    def validate_group(self, group):
        ...
        if (
            group.groupmembership_set.active().editors().count()
            < settings.CONFLICT_RESOLUTION_ACTIVE_EDITORS_REQUIRED_FOR_CREATION
        ):
            raise serializers.ValidationError(
                _(
                    "You need at least %(count)s active trusted users in your group to start this process."
                )
                % {
    ...
    def validate_affected_user(self, affected_user):
        if affected_user == self.context["request"].user:
            raise serializers.ValidationError(
                "You cannot start a conflict resolution about yourself"
            )
    ...
    def validate(self, attrs):
        ...
        if not group.is_member(affected_user):
            raise serializers.ValidationError(
                _("Affected user is not part of that group")
            )
        if (
            Issue.objects.ongoing()
            .filter(group=group, affected_user=affected_user)
            .exists()
        ):
            raise serializers.ValidationError(
                _("A conflict resolution about that user has already been started")
            )
        return attrs
