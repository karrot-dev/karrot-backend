...
class TestEnableIssueNotifications(TestMigrations):
    ...
    def setUpBeforeMigration(self, apps):
        ...
        membership1 = GroupMembership.objects.create(
            group=group, user=user1, notification_types=[]
        )
        membership2 = GroupMembership.objects.create(
            group=group,
            user=user2,
            notification_types=["conflict_resolution", "other", "stuff"],
        )
        self.assertEqual(membership1.notification_types, [])
        self.assertEqual(
            membership2.notification_types, ["conflict_resolution", "other", "stuff"]
        )
