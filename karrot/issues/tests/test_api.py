...
class TestConflictResolutionAPI(APITestCase, ExtractPaginationMixin):
    def setUp(self):
        ...
        self.group = GroupFactory(
            members=[self.member], newcomers=[self.affected_member]
        )
    ...
    def test_create_conflict_resolution_and_vote(self):
        ...
        email_to_editor = next(
            email for email in mail.outbox if email.to[0] == notification_member.email
        )
        ...
        # vote on option
        response = self.client.post(
            "/api/issues/{}/vote/".format(issue["id"]),
    ...
    def test_list_issues_efficiently(self):
        ...
        with self.assertNumQueries(6):
            response = self.get_results(
                "/api/issues/", {"group": self.group.id}, format="json"
            )
...
class TestCaseAPIPermissions(APITestCase, ExtractPaginationMixin):
    ...
    def setUpTestData(cls):
        ...
        cls.group = GroupFactory(
            members=[cls.member, cls.affected_member], newcomers=[cls.newcomer],
        )
    ...
    def test_must_provide_score_for_all_options_in_voting(self):
        ...
        voting = issue.votings.first()
        self.client.force_login(user=self.member)
        response = self.vote_via_API(
            issue, data=[{"option": voting.options.first().id, "score": 1}]
        )
        self.assertEqual(
            response.status_code, status.HTTP_400_BAD_REQUEST, response.data
        )
        self.assertEqual(
            "You need to provide a score for all options",
    ...
    def test_cannot_provide_score_for_options_in_other_voting(self):
        ...
        self.client.force_login(user=self.member)
        response = self.vote_via_API(
            issue, data=[{"option": voting2.options.first().id, "score": 1}]
        )
        self.assertEqual(
            response.status_code, status.HTTP_400_BAD_REQUEST, response.data
        )
        self.assertEqual(
            "Provided option is not part of this voting", response.data[0]["option"][0]
        )
    ...
    def test_removed_member_cannot_access_issue(self):
        ...
        response = self.get_results(
            "/api/conversations/{}/".format(issue.conversation.id)
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND, response.data)
