...
class TestHistoryAPICreateGroup(APITestCase, ExtractPaginationMixin):
    ...
    def test_create_group(self):
        ...
        self.client.post(
            "/api/groups/", {"name": "xyzabc", "timezone": "Europe/Berlin"}
        )
...
class TestHistoryAPIOrdering(APITestCase, ExtractPaginationMixin):
    ...
    def test_ordering(self):
        self.client.force_login(self.member)
        self.client.post(
            "/api/groups/", {"name": "Group 1", "timezone": "Europe/Berlin"}
        )
        self.client.post(
            "/api/groups/", {"name": "Group 2", "timezone": "Europe/Berlin"}
        )
        response = self.get_results(history_url)
...
class TestHistoryAPIWithExistingPickups(APITestCase, ExtractPaginationMixin):
    ...
    def test_dont_modify_pickup(self):
        self.client.force_login(self.member)
        self.client.patch(
            self.pickup_url, {"date": self.pickup.date.as_list()}, format="json"
        )
        response = self.get_results(history_url)
    ...
    def test_join_pickup(self):
        ...
        self.assertEqual(
            parse(response.data[0]["payload"]["date"][0]), self.pickup.date.start
        )
...
class TestHistoryAPIWithDonePickup(APITestCase, ExtractPaginationMixin):
    def setUp(self):
        ...
        self.pickup = PickupDateFactory(
            place=self.place, date=to_range(timezone.now() - relativedelta(days=1))
        )
        self.pickup.add_collector(self.member)
    ...
    def test_pickup_done(self):
        ...
        self.assertLess(
            parse(response.data[0]["date"]), timezone.now() - relativedelta(hours=22)
        )
    ...
    def test_filter_pickup_done(self):
        ...
        response = self.get_results(
            history_url, {"typus": "GROUP_JOIN"}
        )  # unrelated event should give no result
        self.assertEqual(len(response.data), 0)
