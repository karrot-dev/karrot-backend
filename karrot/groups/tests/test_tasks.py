...
from karrot.groups.factories import (
    GroupFactory,
    PlaygroundGroupFactory,
    InactiveGroupFactory,
)
from karrot.groups.models import GroupMembership, GroupStatus
from karrot.groups.tasks import (
...
class TestProcessInactiveUsers(TestCase):
    def setUp(self):
        ...
        self.inactive_membership = set_lastseen_at(
            self.group,
            self.inactive_user,
            days=settings.NUMBER_OF_DAYS_UNTIL_INACTIVE_IN_GROUP + 1,
        )
    ...
    def test_process_inactive_users_sends_email(self):
        ...
        self.assertEqual(
            mail.outbox[0].subject, "{} is missing you!".format(self.group.name)
        )
...
class TestProcessInactiveUsersForRemoval(TestCase):
    ...
    def test_notifies_user_about_removal(self):
        ...
        self.assertEqual(mail.outbox[0].to, [self.inactive_user.email])
        self.assertEqual(
            mail.outbox[0].subject, "{} is really missing you!".format(self.group.name)
        )
...
class TestSummaryEmailTask(TestCase):
    ...
    def make_activity_in_group(self, group):
        ...
        with freeze_time(a_few_days_ago, tick=True):
            ...
            [
                group.conversation.messages.create(author=user, content="hello")
            ...
            [
                {
                    ...
                    "tags": {"group": str(group.id), "group_status": "active",},
                    "fields": {
                        "value": 1,
...
class TestMarkInactiveGroupsTask(TestCase):
    def test_groups_marked_inactive(self):
        ...
        self.assertEqual(
            group_without_recent_activity.status, GroupStatus.INACTIVE.value
        )
