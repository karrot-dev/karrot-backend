...
class TestGroupSummaryEmails(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        m = GroupMembership.objects.get(
            group=cls.group, user=cls.user_without_notifications
        )
    ...
    def test_creates_one_email_per_person(self):
        ...
        self.assertEqual(
            sorted([email.to[0] for email in emails]),
    ...
    def test_creates_one_email_per_person_with_different_languages(self):
        ...
        self.assertEqual(
            sorted(to), sorted([member.email for member in expected_members])
        )
    ...
    def test_ignores_deleted_pickups(self):
        ...
        with freeze_time(a_few_days_ago, tick=True):
            ...
            PickupDateFactory(
                place=place, max_collectors=1, collectors=[user], is_disabled=True
            )
    ...
    def test_group_summary_data(self):
        ...
        with freeze_time(a_couple_of_weeks_ago, tick=True):
            ...
            self.group.conversation.messages.create(
                author=old_user, content="old message"
            )
