...
def prepare_group_summary_data(group, from_date, to_date):
    ...
    pickup_dates = (
        PickupDate.objects.in_group(group)
        .exclude_disabled()
    ...
    feedbacks = Feedback.objects.filter(
        created_at__gte=from_date, created_at__lt=to_date, about__place__group=group,
    )
    ...
    data["has_activity"] = any(
        data[field] > 0 for field in ["pickups_done_count", "pickups_missed_count"]
