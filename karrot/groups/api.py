...
class GroupViewSet(
    ...
    def get_queryset(self):
        ...
        if self.action in ("retrieve", "list"):
            qs = (
                qs.annotate_active_editors_count()
                .annotate_yesterdays_member_count()
                .prefetch_related(
                    "members", "groupmembership_set", "groupmembership_set__trusted_by",
                )
            )
    ...
    def mark_user_active(self, request, pk=None):
        ...
        membership = get_object_or_404(
            GroupMembership.objects, group=pk, user=request.user
        )
    ...
    def trust_user(self, request, pk, user_id):
        ...
        trust, created = Trust.objects.get_or_create(
            membership=membership, given_by=self.request.user,
        )
        if not created:
    ...
    def modify_notification_types(self, request, pk, notification_type):
        ...
        self.check_permissions(request)
        membership = get_object_or_404(
            GroupMembership.objects, group=self.get_object(), user=request.user
        )
        self.check_object_permissions(request, membership)
        ...
        serializer = serializer_class(
            membership, data={"notification_type": notification_type}, partial=True
        )
...
class AgreementViewSet(
    ...
    @action(
        detail=True, methods=["POST"], serializer_class=AgreementAgreeSerializer,
    )
