...
def member_returned(membership):
    ...
    if membership.removal_notification_at is not None:
        ...
        fields.update(
            {
                "group_member_returned_seconds_since_marked_as_inactive": get_seconds_to_now(
                    membership.inactive_at
                )
            }
        )
        ...
        fields.update(
            {
                ...
                "count_active_newcomers_{}d".format(
                    n
                ): active_memberships.newcomers().count(),
                "count_active_editors_{}d".format(
                    n
                ): active_memberships.editors().count(),
                "count_pickup_active_{}d".format(n): pickup_active_memberships.count(),
                "count_pickup_active_newcomers_{}d".format(
                    n
                ): pickup_active_memberships.newcomers().count(),
                "count_pickup_active_editors_{}d".format(
                    n
                ): pickup_active_memberships.editors().count(),
            }
        )
