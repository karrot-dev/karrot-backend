...
def random_issue():
    return IssueFactory(
        group=random_group(), created_by=random_user(), affected_user=random_user()
    )
...
def get_or_create_application():
    ...
    if application is None:
        ...
        if group.application_questions == "":
            ...
            group.save()
        application = ApplicationFactory(group=group, user=new_user,)
...
class Handlers:
    ...
    def accountdelete_success(self):
        return karrot.users.emails.prepare_accountdelete_success_email(
            user=random_user()
        )

    def application_accepted(self):
        application = get_or_create_application()
        return karrot.applications.emails.prepare_application_accepted_email(
            application
        )

    def application_declined(self):
        application = get_or_create_application()
        return karrot.applications.emails.prepare_application_declined_email(
            application
        )
    ...
    def conflict_resolution_continued(self):
        issue = random_issue()
        return karrot.issues.emails.prepare_conflict_resolution_continued_email(
            issue.created_by, issue
        )

    def conflict_resolution_continued_affected_user(self):
        issue = random_issue()
        return karrot.issues.emails.prepare_conflict_resolution_continued_email_to_affected_user(
            issue
        )
    ...
    def new_application(self):
        ...
        return karrot.applications.emails.prepare_new_application_notification_email(
            member, application
        )

    def new_conflict_resolution(self):
        issue = random_issue()
        return karrot.issues.emails.prepare_new_conflict_resolution_email(
            issue.created_by, issue
        )

    def new_conflict_resolution_affected_user(self):
        issue = random_issue()
        return karrot.issues.emails.prepare_new_conflict_resolution_email_to_affected_user(
            issue
        )
    ...
    def passwordreset_success(self):
        return karrot.users.emails.prepare_passwordreset_success_email(
            user=random_user()
        )
    ...
    def user_became_editor(self):
        return prepare_user_became_editor_email(
            user=random_user(), group=random_group()
        )
    ...
    def user_removal_from_group(self):
        return prepare_user_removal_from_group_email(
            user=random_user(), group=random_group()
        )
...
def list_templates(request):
    template_dirs = [
        s for s in get_app_template_dirs("templates") if re.match(r".*/karrot/.*", s)
    ]
                ...
                if re.match(r".*\.jinja2$", filename) and not re.match(
                    ...
                    if name != "template_preview_list":
                        ...
                        for idx, s in enumerate(["subject", "text", "html"]):
                            if os.path.isfile(
                                "{}.{}.jinja2".format(os.path.join(directory, name), s)
                            ):
                                formats.append(s)
                            elif s == "text":
...
def show_template(request):
    ...
    if not has_handler:
        return HttpResponseNotFound(
            "Please setup a handler for the <strong>{}</strong> in <strong>{}</strong>".format(
                name, __file__
            )
        )
    ...
    if format == "html":
        ...
        return HttpResponse(
            "<pre>{}</pre>".format(html.escape(email.message().as_string()))
        )
