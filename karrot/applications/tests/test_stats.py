...
class TestApplicationStats(TestCase):
    def test_application_stats(self):
        ...
        [
            ApplicationFactory(group=group, user=UserFactory(), status="pending")
            for _ in range(3)
        ]
        [
            ApplicationFactory(group=group, user=UserFactory(), status="accepted")
            for _ in range(4)
        ]
        [
            ApplicationFactory(group=group, user=UserFactory(), status="declined")
            for _ in range(5)
        ]
        [
            ApplicationFactory(group=group, user=UserFactory(), status="withdrawn")
            for _ in range(6)
        ]
    ...
    def test_application_status_update(self, write_points):
        ...
        application = ApplicationFactory(
            group=GroupFactory(), user=UserFactory(), created_at=two_hours_ago
        )
