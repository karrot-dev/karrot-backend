...
def prepare_conversation_message_notification(user, messages):
    ...
    raise Exception(
        "Cannot send message notification because conversation doesn't have a known type"
    )
...
def prepare_thread_message_notification(user, messages):
    ...
    reply_to = formataddr(
        (reply_to_name, "{}@{}".format(local_part, settings.SPARKPOST_RELAY_DOMAIN))
    )
...
def prepare_message_notification(
    ...
    *,
    ...
    with translation.override(language_for_user(user)):
        ...
        thread = (
            first_message
            if conversation.target and conversation.target.conversation_supports_threads
            else None
        )
...
def prepare_offer_message_notification(user, messages):
    ...
    with translation.override(language_for_user(user)):
        return prepare_message_notification(
            ...
            group=offer.group,
            conversation_name=_(
                "New message for offer %(offer_name)s in %(group_name)s"
            )
            % {"offer_name": offer.name, "group_name": offer.group.name,},
