...
def get_participants_to_notify(message):
    if message.is_thread_reply():
        participants_to_notify = ConversationThreadParticipant.objects.filter(
            thread=message.thread, muted=False,
        ).filter(
        ...
        participants_to_notify = ConversationParticipant.objects.filter(
            conversation=message.conversation, muted=False,
        )
    ...
    if group is not None:
        ...
        active_members = Q(
            user__groupmembership__inactive_at__isnull=True,
        ...
        participants_to_notify = participants_to_notify.filter(
            active_members | not_in_group
        )

    return (
        participants_to_notify.exclude(user=message.author,)
        .exclude(user__in=User.objects.unverified(),)
        .exclude(seen_up_to__id__gte=message.id,)
        .exclude(notified_up_to__id__gte=message.id,)
        .distinct()
    )
...
def notify_group_conversation_participants(message):
    for participant in get_participants_to_notify(message):
        ...
        send_and_mark(
            participant=participant, message=message, email=email,
        )
...
def notify_participants(message):
    ...
    if (
        ...
        and not message.is_thread_reply()
    ):
    ...
    for participant in get_participants_to_notify(message):
        ...
        email = prepare_conversation_message_notification(
            user=participant.user, messages=messages,
        )
        send_and_mark(
            participant=participant, message=message, email=email,
        )
...
def mark_conversations_as_closed():
    with timer() as t:
        ...
        for conversation in Conversation.objects.filter(
            is_closed=False, target_id__isnull=False,
    ...
    stats_utils.periodic_task(
        "conversations__mark_conversations_as_closed", seconds=t.elapsed_seconds
    )
