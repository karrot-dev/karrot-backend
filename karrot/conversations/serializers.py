...
class ConversationThreadSerializer(serializers.ModelSerializer):
    ...
    def get_participants(self, participant):
        return [
            ...
            for participants in participant.thread.participants.all()
        ]
...
class ConversationMessageSerializer(serializers.ModelSerializer):
    ...
    def get_thread_meta(self, message):
        ...
        participant = next(
            (p for p in message.participants.all() if p.user_id == user.id), None
        )
        if participant:
    ...
    def get_is_editable(self, message):
        return (
            message.is_recent() and message.author_id == self.context["request"].user.id
        )
        ...
        if "thread" in data:
            ...
            if (
                ...
                raise serializers.ValidationError(
                    "You cannot change the thread of a message"
                )
            ...
            if "conversation" in data:
                ...
                if thread.conversation.id != conversation.id:
                    raise serializers.ValidationError(
                        "Thread is not in the same conversation"
                    )

                # only some types of messages can have threads
                if not (
                    conversation.target
                    and conversation.target.conversation_supports_threads
                ):
                    raise serializers.ValidationError(
                        "You can only reply to wall messages"
                    )
...
class ConversationSerializer(serializers.ModelSerializer):
    ...
    participants = serializers.PrimaryKeyRelatedField(
        source="conversation.participants", many=True, read_only=True
    )
    group = serializers.PrimaryKeyRelatedField(
        source="conversation.group", read_only=True
    )
    type = serializers.CharField(source="conversation.type", read_only=True)
    target_id = serializers.IntegerField(
        source="conversation.target_id", read_only=True
    )
    is_closed = serializers.BooleanField(
        source="conversation.is_closed", read_only=True
    )
    notifications = serializers.ChoiceField(
        choices=[(c.value, c.value) for c in ConversationNotificationStatus]
    )
    ...
    def validate_notifications(self, notifications):
        ...
        if (
            participant
            and notifications == ConversationNotificationStatus.NONE.value
        ...
        if (
            ...
            raise serializers.ValidationError(
                "You cannot leave a conversation that is not group public"
            )
