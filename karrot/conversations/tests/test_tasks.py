...
class TestBatchedConversationNotificationTask(TestCase):
    def setUp(self):
        ...
        self.conversation = Conversation.objects.get_or_create_for_two_users(
    ...
    def test_does_notification_batching(self):
        ...
        mail.outbox = []
        with suppressed_notifications():
            two = self.conversation.messages.create(
                author=self.author, content="second message"
            )
            self.conversation.messages.create(author=self.author, content="three")
        self.conversation.conversationparticipant_set.filter(user=self.user).update(
            seen_up_to=two
        )
        with execute_scheduled_tasks_immediately():
            recent_message = self.conversation.messages.create(
        ...
        self.assertEqual(len(mail.outbox), 1)
        user1_email = next(
            email for email in mail.outbox if email.to[0] == self.user.email
        )
...
class TestConversationNotificationTask(TestCase):
    ...
    def test_notify_about_unseen_message(self):
        self.group.conversation.conversationparticipant_set.filter(
            user=self.user
        ).update(seen_up_to=self.message)
        with execute_scheduled_tasks_immediately():
    ...
    def test_exclude_thread_replies_from_conversation_notification(self):
        ...
        with execute_scheduled_tasks_immediately():
            self.group.conversation.messages.create(
                author=self.author, content="conversation"
            )
...
class TestMarkConversationClosedTask(TestCase):
    def test_mark_issue_conversation_as_closed(self):
        ...
        conversation.messages.create(
            content="hello", author=issue.created_by, created_at=long_time_ago
        )
        ...
        conversation_ended_recently.messages.create(
            content="hello",
            author=issue_ended_recently.created_by,
        ...
        conversation_ended.messages.create(
            content="hello", author=issue_ended.created_by, created_at=long_time_ago
        )
    ...
    def test_mark_empty_as_closed(self):
        ...
        conversations = Conversation.objects.filter(target_type__model="issue")
        self.assertEqual(
            conversations.filter(is_closed=True).first(),
            issue_ended_long_ago.conversation,
        )
        self.assertEqual(
            conversations.filter(is_closed=False).first(),
            issue_ended_recently.conversation,
        )
