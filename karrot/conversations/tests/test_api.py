...
class TestConversationsAPI(APITestCase):
    ...
    def test_list_conversations_with_related_data_efficiently(self):
        ...
        with self.assertNumQueries(15):
            response = self.client.get(
                "/api/conversations/", {"group": group.id}, format="json"
            )
    ...
    def test_retrieve_conversation_efficiently(self):
        ...
        with self.assertNumQueries(3):
            response = self.client.get(
                "/api/conversations/{}/".format(conversation.id), format="json"
            )
    ...
    def test_list_only_unread_conversations(self):
        ...
        self.client.force_login(user=self.participant1)
        response = self.client.get(
            "/api/conversations/?exclude_read=True", format="json"
        )
    ...
    def test_list_messages(self):
        ...
        with self.assertNumQueries(5):
            response = self.client.get(
                "/api/messages/?conversation={}".format(self.conversation1.id),
                format="json",
            )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_get_message(self):
        ...
        response = self.client.get(
            "/api/messages/{}/".format(message_id), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_cannot_get_messages_if_not_in_conversation(self):
        self.client.force_login(user=self.participant1)
        response = self.client.get(
            "/api/messages/?conversation={}".format(self.conversation3.id),
            format="json",
        )
        self.assertEqual(
    ...
    def test_create_message(self):
        ...
        self.assertEqual(
            ...
            parse(response.data["created_at"]),
            response.data,
        )
        self.assertEqual(conversation.messages.first().id, response.data["id"])
        self.assertEqual(
            conversation.messages.first().author.id, response.data["author"]
        )
...
class TestGroupPublicConversation(APITestCase):
    def test_can_access_messages_if_not_participant(self):
        ...
        self.client.force_login(user=user)
        response = self.client.get(
            "/api/messages/?conversation={}".format(conversation.id)
        )
...
class TestConversationThreadsAPI(APITestCase):
    ...
    def test_returns_thread_and_replies(self):
        self.client.force_login(user=self.user)
        another_thread = self.conversation.messages.create(
            author=self.user, content="my own thread"
        )
        n = 5
        [self.create_reply(thread=another_thread) for _ in range(n)]

        response = self.client.get(
            "/api/messages/?thread={}".format(another_thread.id), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["results"]), n + 1)

    def test_list_my_recently_active_threads(self):
        most_recent_thread = self.conversation.messages.create(
            author=self.user, content="my own thread"
        )
        ...
        results = response.data["results"]
        self.assertEqual(
            [thread["id"] for thread in results["threads"]],
    ...
    def test_list_only_unread_threads(self):
        read_thread = self.conversation.messages.create(
            author=self.user, content="my own thread"
        )
        ...
        self.client.force_login(user=self.user)
        response = self.client.get(
            "/api/messages/my_threads/?exclude_read=True", format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_can_mute_thread(self):
        ...
        response = self.client.patch(
            "/api/messages/{}/thread/".format(self.thread.id), data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_cannot_mute_thread_with_no_replies(self):
        self.client.force_login(user=self.user)
        another_message = self.conversation.messages.create(
            author=self.user, content="boo"
        )
        data = {"muted": True}
        response = self.client.patch(
            "/api/messages/{}/thread/".format(another_message.id), data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_can_edit_reply(self):
        ...
        response = self.client.patch(
            "/api/messages/{}/".format(reply.id), {"content": "edited!"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_can_react_to_reply(self):
        ...
        response = self.client.post(
            "/api/messages/{}/reactions/".format(reply.id),
            {"name": "smile"},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_cannot_create_private_conversation_threads(self):
        self.client.force_login(user=self.user)
        private_conversation = Conversation.objects.get_or_create_for_two_users(
            self.user, self.user2
        )
        private_message = private_conversation.messages.create(
            author=self.user, content="hey there, you look nice"
        )
        data = {
    ...
    def test_fails_with_incorrect_conversation(self):
        self.client.force_login(user=self.user)
        another_conversation = Conversation.objects.get_or_create_for_two_users(
            self.user, self.user2
        )
        data = {
    ...
    def test_cannot_move_replies_between_threads(self):
        self.client.force_login(user=self.user)
        another_message = self.conversation.messages.create(
            author=self.user, content="yay"
        )
        reply = self.create_reply()
        data = {"thread": another_message.id}
        response = self.client.patch(
            "/api/messages/{}/".format(reply.id), data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class TestConversationsSeenUpToAPI(APITestCase):
    ...
    def test_message_marked_seen_for_author(self):
        ...
        response = self.client.get(
            "/api/conversations/{}/".format(self.conversation.id), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_conversation_list(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["results"]["conversations"][0]["seen_up_to"], message.id
        )
    ...
    def test_mark_seen_up_to(self):
        ...
        response = self.client.patch(
            "/api/conversations/{}/".format(self.conversation.id), data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["seen_up_to"], message.id)
...
class TestConversationsEmailNotificationsAPI(APITestCase):
    ...
    def test_mute(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["notifications"], ConversationNotificationStatus.MUTED.value
        )
    ...
    def test_send_email_notifications(self):
        ...
        with execute_scheduled_tasks_immediately():
            ConversationMessage.objects.create(
                author=self.user, conversation=self.conversation, content="asdf"
            )
...
class TestConversationsMessageReactionsPostAPI(APITestCase):
    def setUp(self):
        ...
        self.message = self.conversation.messages.create(
            author=self.user, content="hello"
        )
        ...
        self.message2 = self.conversation2.messages.create(
            author=self.user, content="hello2"
        )
    ...
    def test_not_logged(self):
        ...
        data = {"name": "thumbsup"}
        response = self.client.post(
            "/api/messages/{}/reactions/".format(self.message.id), data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_add_emoji_to_message_of_other_group(self):
        ...
        data = {"name": "thumbsup"}
        response = self.client.post(
            "/api/messages/{}/reactions/".format(self.message2.id), data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_add_to_message_with_invalid_id(self):
        ...
        data = {"name": "thumbsup"}
        response = self.client.post(
            "/api/messages/{}/reactions/".format("invalid"), data, format="json"
        )
    ...
    def test_empty_request_fails(self):
        ...
        self.client.force_login(user=self.user)
        response = self.client.post(
            "/api/messages/{}/reactions/".format(self.message.id), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_reaction_to_nonexistent_message(self):
        ...
        data = {"name": "thumbsup"}
        response = self.client.post(
            "/api/messages/{}/reactions/".format(1735), data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_react_to_message_with_emoji(self):
        ...
        self.assertTrue(
            ConversationMessageReaction.objects.filter(
                user=self.user, message=self.message, name="tada"
            ).exists()
        )
    ...
    def test_include_reactions_in_message(self):
        ...
        self.client.force_login(user=self.user)
        response = self.client.get(
            "/api/messages/?conversation={}".format(self.conversation.id), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["results"]), 1)
...
class TestConversationsMessageReactionsDeleteAPI(APITestCase):
    ...
    def test_remove_reaction_invalid_message_id(self):
        ...
        response = self.client.delete(
            "/api/messages/{}/reactions/{}/".format("hello", "thumbsup"), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_remove_reaction_message_not_exist(self):
        ...
        response = self.client.delete(
            "/api/messages/{}/reactions/{}/".format(7321, "thumbsup"), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class TestConversationsMessageEditAPI(APITestCase):
    ...
    def test_edit_message(self):
        ...
        now = timezone.now()
        response = self.client.patch(
            "/api/messages/{}/".format(self.message.id), data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
    ...
    def test_cannot_update_message_if_not_message_author(self):
        ...
        response = self.client.patch(
            "/api/messages/{}/".format(self.message2.id), data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_cannot_update_message_if_past_10_days(self):
        ...
        data = {"content": "a nicer message"}
        response = self.client.patch(
            "/api/messages/{}/".format(self.message3.id), data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class TestClosedConversation(APITestCase):
    def setUp(self):
        ...
        self.conversation = ConversationFactory(
            participants=[self.user], is_closed=True
        )

    def test_write_message_in_closed_conversation_fails(self):
        self.client.force_login(user=self.user)
        response = self.client.post(
            "/api/messages/", {"conversation": self.conversation.id, "content": "hello"}
        )
...
class TestPrivateConversationAPI(APITestCase):
    ...
    def test_cannot_leave_private_conversation(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.data["notifications"], ["You cannot leave a private conversation"]
        )
