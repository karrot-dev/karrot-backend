...
class ConversationFilter(filters.FilterSet):
    exclude_read = filters.BooleanFilter(
        field_name="unread_message_count", method="filter_exclude_read"
    )
...
class ConversationViewSet(
    mixins.RetrieveModelMixin, PartialUpdateModelMixin, GenericViewSet
):
    """
    Conversations
    """
    ...
    def get_queryset(self):
        ...
        if self.action == "retrieve":
            qs = qs.select_related(
                "conversation", "conversation__target_type"
    ...
    def get_object(self):
        try:
            ...
            queryset = Conversation.objects.filter(
                group__groupmembership__user=self.request.user, is_group_public=True,
            )
            pk = self.kwargs["pk"]
    ...
    def list(self, request, *args, **kwargs):
        queryset = (
            ...
            .annotate_unread_message_count()
            .annotate(
                conversation_latest_message_id=F("conversation__latest_message_id")
            )
            .select_related(
                "conversation",
            ...
            .prefetch_related(
                "conversation__latest_message__reactions", "conversation__participants",
            )
        ...
        messages = [
            c.latest_message for c in conversations if c.latest_message is not None
        ]
        ...
        pickups = PickupDate.objects.filter(
            id__in=[c.target_id for c in pickup_conversations]
        ...
        application_conversations = [
            item for item in conversations if item.target_type == applications_ct
        ]
        ...
        issue_conversations = [
            item for item in conversations if item.target_type == issues_ct
        ]
        issues = Issue.objects.filter(
            id__in=[c.target_id for c in issue_conversations]
        ).prefetch_for_serializer(user=request.user)

        offers_ct = ContentType.objects.get_for_model(Offer)
        offer_conversations = [
            item for item in conversations if item.target_type == offers_ct
        ]
        offers = Offer.objects.filter(
            id__in=[c.target_id for c in offer_conversations]
        ...
        def get_conversation(application):
            return next(
                c for c in application_conversations if c.target_id == application.id
            )

        users = (
            get_user_model()
            .objects.filter(
                conversationparticipant__conversation__in=[
                    get_conversation(a) for a in my_applications
                ]
            )
        ...
        message_serializer = ConversationMessageSerializer(
            messages, many=True, context=context
        )
        ...
        meta_serializer = ConversationMetaSerializer(
    ...
    def mark_threads_seen(self, request):
        ...
        self.check_permissions(request)
        meta, _ = ConversationMeta.objects.update_or_create(
            {"threads_marked_at": timezone.now()}, user=request.user
        )
        serializer = ConversationMetaSerializer(meta)
        return Response(serializer.data)
...
class ConversationMessageFilter(filters.FilterSet):
    ...
    exclude_read = filters.BooleanFilter(
        field_name="unread_replies_count", method="filter_exclude_read"
    )
...
class ConversationMessageViewSet(
    ...
    def get_queryset(self):
        ...
        qs = (
            self.queryset.with_conversation_access(self.request.user)
            .annotate_replies_count()
            .annotate_unread_replies_count_for(self.request.user)
        )

        if self.action == "my_threads":
            return qs.only_threads_with_user(self.request.user).prefetch_related(
                "participants", "latest_message"
            )
    ...
    def my_threads(self, request):
        ...
        return paginator.get_paginated_response(
            {"threads": serializer.data, "messages": message_serializer.data}
        )
    ...
    @action(
        detail=True, methods=["PATCH"], serializer_class=ConversationThreadSerializer
    )
    ...
    def remove_reaction(self, request, pk, name):
        ...
        reaction = get_object_or_404(
            ConversationMessageReaction, name=name, message=message, user=request.user
        )
...
class RetrieveConversationMixin(object):
    ...
    def retrieve_conversation(self, request, *args, **kwargs):
        ...
        participant = conversation.conversationparticipant_set.filter(
            user=request.user
        ).first()
        if not participant:
            if conversation.can_access(request.user):
                ...
                self.permission_denied(
                    request, message=_("You are not in this conversation")
                )
...
class RetrievePrivateConversationMixin(object):
    ...
    def retrieve_private_conversation(self, request, *args, **kwargs):
        ...
        try:
            conversation = Conversation.objects.get_or_create_for_two_users(
                request.user, user2
            )
        except Exception:
