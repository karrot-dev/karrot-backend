...
class TestActivityAPICreateGroup(PaginatedResponseTestCase):
    @classmethod
    ...
    def test_create_group(self):
        ...
        response = self.get_results(activity_url)
        self.assertEqual(response.data[0]['typus'], 'GROUP_CREATE')


class TestActivityAPIOrdering(PaginatedResponseTestCase):
    @classmethod
    def setUpTestData(cls):
        cls.member = UserFactory()
...
class TestActivityAPIWithExistingGroup(PaginatedResponseTestCase):
    @classmethod
    def setUpTestData(cls):
        cls.member = UserFactory()
...
class TestActivityAPIWithExistingStore(PaginatedResponseTestCase):
    @classmethod
    def setUpTestData(cls):
        cls.member = UserFactory()
        cls.group = GroupFactory(members=[cls.member, ])
...
class TestActivityAPIWithExistingPickups(PaginatedResponseTestCase):
    @classmethod
    def setUpTestData(cls):
        cls.member = UserFactory()
        cls.group = GroupFactory(members=[cls.member, ])
...
class TestActivityAPIWithDonePickup(PaginatedResponseTestCase):
    @classmethod
    def setUpTestData(cls):
        cls.member = UserFactory()
        cls.group = GroupFactory(members=[cls.member, ])
    ...
    def test_filter_pickup_done(self):
        self.client.force_login(self.member)
        response = self.get_results(activity_url, {'typus': 'PICKUP_DONE'})
        self.assertEqual(response.data[0]['typus'], 'PICKUP_DONE')
        response = self.get_results(activity_url, {'typus': 'GROUP_JOIN'})  # unrelated event should give no result
        self.assertEqual(len(response.data), 0)


class TestActivityAPIWithMissedPickup(PaginatedResponseTestCase):
    @classmethod
    def setUpTestData(cls):
        cls.member = UserFactory()
        cls.group = GroupFactory(members=[cls.member, ])
    ...
    def test_filter_pickup_done(self):
        self.client.force_login(self.member)
        response = self.get_results(activity_url, {'typus': 'PICKUP_MISSED'})
        self.assertEqual(response.data[0]['typus'], 'PICKUP_MISSED')
        response = self.get_results(activity_url, {'typus': 'GROUP_JOIN'})  # unrelated event should give no result
        self.assertEqual(len(response.data), 0)


class TestActivityAPIDateFiltering(PaginatedResponseTestCase):
    @classmethod
    def setUpTestData(cls):
        cls.member = UserFactory()
    ...
    def test_filter_by_date(self):
        self.client.force_login(self.member)
        self.client.post('/api/groups/', {'name': 'xyzabc', 'timezone': 'Europe/Berlin'})
        response = self.get_results(activity_url, data={'date_1': timezone.now()})
        self.assertEqual(len(response.data), 1)
        response = self.get_results(activity_url, data={'date_0': timezone.now()})
        self.assertEqual(len(response.data), 0)
