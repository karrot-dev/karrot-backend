...
class ContextFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        record.port = ','.join([str(p) for p in self.local_ports])
        return True
...
class ProcessSetup(multiprocessing.Process):
    ...
    def common_setup(self):
        ...
        settings.DEBUG = True
        setup_logger('django.channels', verbosity=2)
        ...
        formatter = logging.Formatter(
            fmt='Worker %(pid)s - Port %(port)s - %(message)s')
        handler = logging.StreamHandler()
        logger.addHandler(handler)
        handler.setFormatter(formatter)
        self.logContext = ContextFilter()
        logger.addFilter(self.logContext)

        for alias in self.databases:
            print(connections[alias])

        connection_created.connect(self.cache_connection)

        self.override_settings()

    def cache_connection(self, **kwargs):
        p = psutil.Process(os.getpid())
        self.logContext.local_ports = [c.laddr.port for c in p.connections() if c.raddr.port == 5432]
        print('setting ports', self.logContext.local_ports)
...
class ChannelLiveServerTestCase(TransactionTestCase):
    ...
    def _pre_setup(self):
        ...
        for _ in range(self.worker_processes):
            ...
            worker_process = self.WorkerProcess(
                worker_ready,
                self.worker_threads,
                self._overridden_settings,
                self._modified_settings,
